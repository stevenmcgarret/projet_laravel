...
<body>
    ...
    <div class="d-flex justify-content-center align-items-center vh-100 bg-light">
        <div class="w-50 p-5 bg-white rounded-4 border" style="box-shadow: 0 8px 30px rgba(0,0,0,0.15);">
            ...
            <form action="{{ route("save_reparateur", ['id'=>$reparateur->id]) }}" method="POST">
                @csrf
                <div class="mb-3">
                    <label for="nom" class="form-label">Nom</label>
                    <input type="text" value="{{ $reparateur->nom }}" class="form-control" id="nom" name="nom" aria-describedby="emailHelp"
                        placeholder="Nom du réparateur">
                </div>
                <div class="mb-3">
                    <label for="prenom" class="form-label">Prénom</label>
                    <input type="text" value="{{ $reparateur->prenom }}" class="form-control" id="prenom" name="prenom"
                        placeholder="Prénom du réparateur">
                </div>
                <div class="mb-3">
                    <label for="specialite" class="form-label">Spécialité</label>
                    <input type="text" value="{{ $reparateur->specialite }}" class="form-control" id="specialite" name="specialite"
                        placeholder="Spécialité du réparateur">
                </div>
